...
const About = () => {
  ...
  const { placeholderImage } = useStaticQuery(
    graphql`
      query {
        placeholderImage: file(relativePath: { eq: "palm-coast-bg.jpg" }) {
          childImageSharp {
            gatsbyImageData(
              quality: 50, 
...
text-3xl font-bold text-almond
`
const IntroParagraph = tw(props => <p {...props}/>)`
my-4 text-fogra lg:max-w-md
`
...
max-w-sm mx-4 mt-4 overflow-hidden bg-almond-light rounded-2xl shadow-md border-2
`
...
text-xs font-medium text-almond uppercase border-2
`
const CardContentWrapper = tw.div`
mt-4 border-2
`
const TitleLink = tw.a`
block mt-2 text-2xl font-semibold text-fogra hover:text-ming-light hover:underline
`
const CardText = tw.div`
mt-2 text-sm text-fogra border-2
`
...
mx-1 text-xs text-fogra border-2
`